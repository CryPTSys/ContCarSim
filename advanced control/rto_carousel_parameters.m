function p = rto_carousel_parameters(p)
    ...
    p.visc_liq_coeff=[-3.1970 7.4084E+02 4.6291E-03 -7.1715E-06];          % coefficients component 2        
    ...
    p.coeff_antoine= [74.475, -7164.3, -7.327, 3.1340e-6, 2];      % coefficients component 1                         
    ...
    p.MW_air = 28.97*1e-3;                 % air molecular weight [kg/mol]
    ...
    %% Kinetics
    p.wl_crit = 0.05;    % Drying - critical impurity content [m3_i/m3]
    p.wl_eq = 5e-4;%5e-4;      % Drying - equilibrium impurity content [m3_l/m3] 
    p.zeta=199.78;
    
    %% Calculation of cake properties 
    p.E=0.35;
    p.alpha=2.7e9;
    p.k=1/(p.alpha*p.rho_sol*(1-p.E)); 
    p.a_V=126000;
    p.S_inf=0.085;
