function [u,operating_vars] = controller_online_Layer2(process_time,cycle_time,...
    ...
    %% Paper control
    if control_mode == 1 && stations_working(4)==1 % Layer 1 - for u_nominal.V_slurry = 3e-6
    ...
    elseif control_mode == 2 && stations_working(4)==1 % Layer 1 - for u_nominal.V_slurry = 6e-6           
        if measurements.Tg_out_TI102(end)<18.7+273.15 || ...
            measurements.Tg_out_TI102(end)<=measurements.Tg_out_TI102(end-40)               
            u.t_cycle=cycle_time+1;
        else
            u.t_cycle=cycle_time;
        end
    elseif control_mode >= 3.2 && stations_working(4)==1   % Layer 2 - end-point controller with EKF         
        if x_estim.(['batch_' num2str(n_cycle-3)]).mass_avg(end) + ...
                2*x_estim.(['batch_' num2str(n_cycle-3)]).mass_avg_std(end)<0.005
            u.t_cycle=cycle_time;
        else
            u.t_cycle=cycle_time+1;
        end        
    elseif control_mode ~= 3.1 % Layer 0
