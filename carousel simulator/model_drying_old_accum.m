function drying_output = model_drying(deliq_output,p)
...
%     flux_eq3=fluxes_HRFVM_vL(x_j(1:p.number_nodes,3).*cp_gas_j,p);
    ...
    MB_solv_G= (-ug_j*rho_g_j.*dflux_eq1_dz+DR_j)./(rho_g_j.*eps_g_j); % ODE - solvent concentration in G phase
    MB_L=-DR_j/p.rho_liq; % ODE - MB L phase
    EB_G= (-ug_j*rho_g_j.*dflux_eq3_dz-h_T_j*p.a_V.*(x_j(:,3)-x_j(:,4)))./(rho_g_j.*eps_g_j.*cp_gas_j); % ODE - air inlet temperature
    EB_S=(-DR_j*p.latent_heat+h_T_j*p.a_V.*(x_j(:,3)-x_j(:,4)))./(p.cp_s*...
        p.rho_sol*(1-p.E)+p.cp_l*p.rho_liq*x_j(:,2)); % ODE - EB L+S
