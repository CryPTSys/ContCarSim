function max_impurity=carousel_simulator_funct_sens(cryst_output,p)
        
        % Initialization
        sequence=[]; % vector collecting sequence of processing steps and corresponding duration for the current batch
        
        % for DS calculation, the time steps for calculating dynamic
        % profiles of state variables during the processing steps are not
        % used: only the final state is of interest
%         p.time_step_deliq=p.t_rot; % Time step for output saturation profile of deliquoring [s]
%         p.time_step_filt=p.t_rot; % Time step for output saturation profile of filtration [s]
%         p.time_step_drying=p.t_rot; % Time step for output profiles of filtration [s]
...
%                 output_pos2=model_deliquoring_species_grad(washing_output,residual_time_pos2,p); 
    ...
    vol_cont_impurities=mean(drying_output.vol_cont_impurities,2);
    max_impurity=max(vol_cont_impurities); % for DS calculation, we are only interested in this output
