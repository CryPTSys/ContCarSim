...
%     t_filt=t_filt(t_filt<filtration_duration);  % filtration time
    ...
    p.L_cake=H_cake(end); 
    p.Pb=sum(p.pb_CSD(cryst_output.x_perc).*cryst_output.CSD_perc);
    ...
    p.S_inf=sum(0.155*(1+0.031*p.N_cap_CSD(cryst_output.x_perc,rho_liq).^(-0.49)).*cryst_output.CSD_perc);    % irreducible cake saturation (minimum saturation that can be achieved by displacement of the interstitial liquid by the applied vacuum    
