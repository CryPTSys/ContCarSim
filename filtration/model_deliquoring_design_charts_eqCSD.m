function d = model_deliquoring_design_charts_eqCSD(cryst_output,filt_output,p)
    ...
    N_cap_CSD=@(x) (p.E^3*(x).^2*(p.rho_liq*9.81.*L_cake+p.dP))/((1-p.E)^2*L_cake.*p.surf_t);
    S_inf=trapz(cryst_output.x,0.155*(1+0.031*N_cap_CSD(cryst_output.x).^(-0.49)).*cryst_output.CSD'/p.m0);    % irreducible cake saturation (minimum saturation that can be achieved by displacement of the interstitial liquid by the applied vacuum    
