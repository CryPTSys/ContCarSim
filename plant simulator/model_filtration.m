function [x,y]=model_filtration(cycle_time,Dt,p,u,x,y,n_cycle,pos) 
    ...
    if pos > 1
        x.(['pos' num2str(pos)]).V_filt0=-y.(['pos' num2str(pos-1)]).(['cycle_' num2str(n_cycle)]).V_filt(end);
        x.(['pos' num2str(pos)]).t_filt0=-y.(['pos' num2str(pos-1)]).(['cycle_' num2str(n_cycle)]).t_filt(end);
    else
