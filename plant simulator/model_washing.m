function [x,y] = model_washing(cycle_time,Dt,p,u,x,y,n_batch,pos)
    %%  Inputs list
    %   cycle_time - unused
    ...
    y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).V_filt=...
            [y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).V_filt ...
            V_filt(2:end)+y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).V_filt(end)];
...
function c=species_balance(c0,c_inlet,Dl,S,W,v,p,xx)
    ...
    c_saturated_cake=c0+(c_inlet-c0).*0.5.*(erfc((z/xx.L_cake-W_lambda)./...
                        (2*sqrt(W_lambda)).*sqrt(v*xx.L_cake./Dl))+exp(v*z./Dl).*...
                        erfc((z/xx.L_cake+W_lambda)./(2*sqrt(W_lambda)).*sqrt(v*xx.L_cake./Dl)));
    ...
    c=c0+(c_inlet-c0).*0.5.*((erfc((z/xx.L_cake-Wcorr)./(2*sqrt(Wcorr)).*...
        sqrt(v*xx.L_cake./Dl))+exp(v*z./Dl).*erfc((z/xx.L_cake+Wcorr)./...
                    (2*sqrt(Wcorr)).*sqrt(v*xx.L_cake./Dl))));
