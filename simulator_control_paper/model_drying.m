function [x,y] = model_drying(cycle_time,Dt,p,d,u,x,y,n_batch,pos)
    ...
    if Dt > 0    % drying occurs
        ...
        y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).Ts=...
            [y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).Ts, Tsolid(2:end,end)'];
        y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).wg=...
            [y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).wg,...
            mass_frG(2:end,[1:p.number_volatile_components]*xx.number_nodes_drying)'];
        y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).impurity_content=...
            [y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).impurity_content,...
            composition(2:end)'];
        y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).Tin=...
            [y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).Tin, Tin(2:end)];
        y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).Vdryer=...
            [y.(['pos' num2str(pos)]).(['cycle_' num2str(n_batch)]).Vdryer, (1000*60*ug0*p.A*ones(1,length(Tin)-1))];
